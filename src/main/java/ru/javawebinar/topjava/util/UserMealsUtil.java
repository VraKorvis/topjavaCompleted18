...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededWithMerge(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, Integer> userMealMapGroupedByDate = new HashMap<>();

        mealList.forEach((userMeal) -> userMealMapGroupedByDate.merge(userMeal.getDateTime().toLocalDate(), userMeal.getCalories(), Integer::sum));

        mealList.forEach((userMeal) -> {

            boolean isExceed = userMealMapGroupedByDate.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay;
